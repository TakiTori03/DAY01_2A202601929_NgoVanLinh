...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        gpt4o_res = result.get("gpt4o_answer", result.get("gpt4o_response", ""))
        mini_res = result.get("mini_answer", result.get("mini_response", ""))
        gpt4o_lat = result.get("gpt4o_time", result.get("gpt4o_latency", 0.0))
        mini_lat = result.get("mini_time", result.get("mini_latency", 0.0))

        rows.append(
            " | ".join(
                [
                    shorten(str(result.get("prompt", ""))),
                    shorten(str(gpt4o_res)),
                    shorten(str(mini_res)),
                    f"{gpt4o_lat:.2f}s",
                    f"{mini_lat:.2f}s",
                ]
            )
